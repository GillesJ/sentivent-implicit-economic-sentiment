#!/usr/bin/env python3
'''
Adapted from simpletransformers to change

custom_classification_model.py in sentivent-implicit-economic-sentiment
...
MODELS_WITH_ADD_PREFIX_SPACE = ["roberta", "roberta-lexicon", "camembert", "xlmroberta", "longformer", "mpnet"]
...
def build_lexicon_classification_dataset(data, tokenizer, args, mode, multi_label, output_mode, no_cache):
    ...
    if os.path.exists(cached_features_file) and (
        ...
        if len(data) == 3:
            text_a, labels, lexicon_features = data
            text_b = None
        elif len(data) == 4:
            # Sentence pair task
        ...
        examples['lexicon_features'] = torch.FloatTensor(lexicon_features)
...
class LexiconClassificationDataset(Dataset):
    '''
    Adapted from simpletransformers.classification.classification_utils
    '''
    def __init__(self, data, tokenizer, args, mode, multi_label, output_mode, no_cache):
        self.examples, self.labels = build_lexicon_classification_dataset(
...
class CustomClassificationModel:
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes a CustomClassificationModel model.
        ...
        if self.args.labels_list:
            ...
            if self.args.labels_map:
                try:
                    assert list(self.args.labels_map.values()) == self.args.labels_list
        ...
        self.config.lexicon_feat_dim = self.args.lexicon_feat_dim
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.args.use_hf_datasets:
            ...
            if "text" in train_df.columns and "labels" in train_df.columns: # REGULAR LOADING
                if self.args.model_type == "layoutlm":
                    train_examples = [
                ...
                else: # ON PATH Regular use-case
                    train_examples = (
                        train_df["text"].astype(str).tolist(),
                        train_df["labels"].tolist(),
                        train_df['lexicon_features'].tolist()
                    )
            ...
            train_dataset = self.load_and_cache_examples(train_examples, verbose=verbose) # ON PATH
            ...
            if "text" in eval_df.columns and "labels" in eval_df.columns:
                if self.args.model_type == "layoutlm":
                    ...
                    eval_examples = (
                        ...
                        eval_df['lexicon_features'].tolist(),
                    )
    ...
    def load_and_cache_examples(
        ...
        if args.sliding_window or self.args.model_type == "layoutlm":
            ...
            if os.path.exists(cached_features_file) and (
                ...
                if self.args.model_type != "layoutlm" or 'lexicon' in self.args.model_type:
                    if len(examples) == 3:
                        examples = [
            ...
            dataset = LexiconClassificationDataset( # REGULAR USE
                examples,
                self.tokenizer,
                self.args,
                mode=mode,

...
class TransformerClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.lexicon = self.config.training['lexicon']
        self.lexicon_size = self.config.training['lexicon_size']
...
class TransformerTokenizer:
    ...
    def __init__(self, model):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model.weights, use_fast=False)
        if self.distributed and self.local_rank == 0:
            torch.distributed.barrier()

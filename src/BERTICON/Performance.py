...
class Performance:
    ...
    def evaluate(self, epoch):
        ...
        return avg_train_loss, train_2nd_metric[0] if self.num_labels == 1 else train_2nd_metric, fig, save_model, done_training
                ...
                if self.task == 'regression':
                    ...
                    if self.num_labels == 1:
                        ...
                        for i in range(self.num_labels):
                            labels_i = [item[i] for item in self.gathered[partition]['labels']]
                            preds_i = [item[i] for item in self.gathered[partition]['preds']]
                            r_i = pearsonr(labels_i, preds_i)
                            pearsonrs.append(r_i[0])
                        metric_res = np.mean(pearsonrs)
                else:
                    # Returns a float
                    #print(self.gathered[partition]['labels'])
                    #print(self.gathered[partition]['preds'])

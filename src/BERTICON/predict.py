    ...
    def predict(self):
        ...
        while next(self.config):
            ...
            if self.config.process['is_first']:
                ...
                model_out = self.output_p.joinpath('model.pth')
                ...
                with self.output_p.joinpath('config.json').open('w', encoding='utf-8') as fhout:
                    json.dump(self.config.config, fhout)

                # move output model to output_dir
                move('tmp-checkpoint.pth', model_out)
                # Save plot
                fig.savefig(self.output_p.joinpath('plot.png'))

                if report:
                    with self.output_p.joinpath('report.txt').open('w', encoding='utf-8') as fhout:
                        fhout.write(report)
